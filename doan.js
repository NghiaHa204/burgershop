...
function checkregister(event) {
  ...
  if (!regexPassword.test(passInput.value)) {
    passCheck.innerText =
      "Mật khẩu phải từ 8 kí tự, có một chữ hoa, kí tự đặc biệt, chữ số!";
    passInput.focus();
    return false;
  }

  var a = retypePassInput.value;
  var b = passInput.value;
...
function checkPermission() {
  if (localStorage.getItem("userlogin")) {
    ...
    if (user.username == "admin") {
      s =
        `<span style="display: none;" id="input_span" >
...
async function hien_thi_san_pham(type) {
  ...
  if (type.toString() === "all") {
    ...
    let menuList = document.querySelector(
      "#thuc_don .page_main_container .menu_list_ul"
    );
      ...
      addcontent.innerHTML = `
                <div class="menu_list_product" id="${product.maSanPham}">
                    <a href="#" class="menu_list_img">
                        <img src="${
                          product.hinhAnh
                        }" onclick="get_order()" alt="${product.tenSanPham}">
                    </a>
    ...
    close_search_input();
    let menuList = document.querySelector(
      "#hoanh_chicken .page_main_container .menu_list_ul"
    );
...
function get_element(menu_list_product) {
  ...
  var decrible = menu_list_product.querySelector(
    ".menu_list_decrible"
  ).innerText;
...
function change_release() {
  ...
  document.getElementById("Sum_money").innerText =
    (new_price * 1000).toLocaleString() + " VND";
}
...
function change_cart_release() {
  ...
  var price = parseFloat(
    element
      .querySelector(".cart_sum_money .cart_sum_money_p ")
...
function tmp_monney() {
  ...
  for (var i = 0; i < len.length; i++) {
    tmp_price +=
      parseFloat(len[i].innerText.replace(" VND", "")) * coun[i].value;
  }
  document.getElementById("cart_tmp_cal_money").innerText =
    (tmp_price * 1000).toLocaleString() + " VND";
  document.getElementById("thanh_toan_tmp_cal_money").innerText =
    (tmp_price * 1000).toLocaleString() + " VND";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  click_add.addEventListener("click", function () {
    ...
    price =
      (parseFloat(price.replace(/[^\d]/g, "")) / count).toLocaleString() +
      " VND";
...
function van_chuyen() {
  ...
  switch (choose) {
    case 1:
      phi.innerText = 10000 + " VND";
      break;
    case 2:
      phi.innerText = 20000 + " VND";
      break;
    case 3:
      phi.innerText = 30000 + " VND";
      break;
  }
...
function tong_cong() {
  return (document.getElementById("tong_cong").innerText =
    (tmp_monney() + van_chuyen()).toLocaleString() + " VND");
}
...
function deleteRow_user_bill(button) {
  ...
  if (allBills.hasOwnProperty(username)) {
    ...
    for (let i = 0; i < userBills.length; i++) {
      if (
        userBills[i].trang_thai &&
        userBills[i].trang_thai.toString() !== "Đã hủy" &&
        userBills[i].timeid === billid
      ) {
    ...
    const updatedBills = userBills.filter(
      (item) => item.timeid.toString() !== billid
    );
...
async function open_user_bill() {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/api/user-bills/${userId}`
    );
    ...
    if (result.success) {
        ...
        if (billItem.timeid !== previousTimeId) {
          ...
          row.innerHTML = `
                        ...
                        <td>
                            <select id="id${
                              billItem.timeid
                            }" class="trang_thai" style='pointer-events: none;'>
                                <option value="1" ${
                                  billItem.trang_thai === "Vừa đặt"
                                    ? "selected"
                                    : ""
                                }>Vừa đặt</option>
                                <option value="2" ${
                                  billItem.trang_thai === "2" ? "selected" : ""
                                }>Đang giao</option>
                                <option value="3" ${
                                  billItem.trang_thai === "3" ? "selected" : ""
                                }>Đã giao</option>
                                <option value="4" ${
                                  billItem.trang_thai === "4" ? "selected" : ""
                                }>Đã nhận</option>
                                <option value="5" ${
                                  billItem.trang_thai === "5" ? "selected" : ""
                                }>Đã hủy</option>
                            </select>
                        </td>
                        <td>
      ...
      console.error(
        "Không thể lấy danh sách hóa đơn của người dùng:",
        result.message
      );
    }
  } catch (error) {
...
function show_item_search() {
  ...
  for (let i = 0; i < item.length; i++) {
    ...
    addcontent.classList.add("menu_list_li");
    addcontent.innerHTML = `
                    <div class="menu_list_product" id=${
                      item[i].type.toString() + item[i].id.toString()
                    } >
                        <a href="#" class="menu_list_img">
                            <img src="${
                              item[i].image
                            }" onclick="get_order()" alt="${item[i].title}">
                        </a>
                        <h4 class="menu_list_title"><a href="#" onclick="get_order()">${item[
                          i
                        ].title.toUpperCase()}</a></h4>
  ...
  if (searchTerm && listItems.length > 0) {
    ...
    if (search_item.length > 0) {
      ...
      document.getElementById(
        "content_page"
      ).innerHTML = `<p style="text-align: center;">Không tìm thấy sản phẩm nào</p>`;
      menuList.innerHTML = `
...
async function sap_xep_product_price_up() {
  try {
    ...
    productList.sort(
      (a, b) =>
        parseFloat(a.price.replace(" VND", "")) -
        parseFloat(b.price.replace(" VND", ""))
    );
...
function check_id_product(type) {
  ...
  while (i <= 100) {
    const existsInProductList = data.some(
      (item) => item !== null && item.id === i
    );
...
function add_product_to_UI(product) {
  ...
  addcontent.innerHTML = `
        <td class="new-cart-list-img">
            <img style="border-radius: 10px;" class="new-cart-img" src="${
              product.image
            }" alt="Image">
        </td>
        <td class="new-Bill-check">
            <p class="new-cart-title" id="${
              product.type.toString() + product.id.toString()
            }">${product.title}</p>
        </td>
        <td class="cart-sum-money">
            <h4 class="new-cart-sum-money" id="new-cart-sum-money">${
              product.price
            }</h4>
        </td>
        <td>
...
function delete_item(productId) {
  ...
  const index = productList.findIndex(
    (product) => product.type.toString() + product.id.toString() === productId
  );
...
async function edit_item() {
  ...
  const index = productList.findIndex(
    (product) =>
      product.type.toString() + product.id.toString() === id.toString()
  );

  for (let i = 0; i < productList.length; i++) {
    if (
      (id.toString() ===
        productList[i].type.toString() + productList[i].id.toString() &&
        id !== currentId) ||
      !isValidString(id)
    ) {
      document.getElementById(
        "productId_edit_small"
      ).innerText = `ID mới không hợp lệ, bị trùng hoặc sai định dạng (id phải có dạng 'loại sản phẩm + 1 số nguyên ví dụ: combo1) 
      ...
      document.getElementById("productId_edit_small").innerText =
        "ID mới hợp lệ";
    }
  }
  ...
  if (!imageDataURL || imageDataURL == "") {
    document.getElementById("productImage_edit_small").innerText =
      "Thiếu tệp ảnh (ảnh không được để trống)";
  ...
  //kiểm tra tiêu đề
  if (!title) {
    document.getElementById("productTitle_edit_small").innerText =
      "Tiêu đề không hợp lệ (tiêu đề không được để trống)";
  ...
  //kiểm tra mô tả
  if (!content) {
    document.getElementById("productContent_edit_small").innerText =
      "Mô tả không hợp lệ (mô tả không được để trống)";
  ...
  if (isNaN(price) || parseFloat(price) <= 0) {
    document.getElementById("productPrice_edit_small").innerText =
      "Giá Bán không hợp lệ (giá bán phải là 1 số nguyên > 0 và không có lẫn các kí tự đặt biệt)";
...
function edit_product(button) {
  ...
  if (row) {
    ...
    if (pElement && pElement.id) {
      ...
      document.getElementById("productId_edit").value = currentId =
        data.type.toString() + data.id.toString();
      document.getElementById("productTitle_edit").value = data.title;
      document.getElementById("productContent_edit").value = data.content;
      document.getElementById("productPrice_edit").value =
        parseFloat(data.price.replace(" VND", "")) * 1000;
    }
  }
...
async function searchUser() {
  ...
  try {
    ...
    List.innerHTML =
      "<tr><td colspan='5'>Lỗi xảy ra khi tìm kiếm. Vui lòng thử lại.</td></tr>";
  }
}
...
async function thanh_toan() {
  ...
  const timeid =
    date.getHours() + "" + date.getMinutes() + "" + date.getSeconds(); // Unique ID
  ...
  for (let i = 0; i < product.length; i++) {
    ...
    if (titleElem && countElem && priceElem && totalMoneyElem) {
      ...
      const total_money = parseFloat(
        totalMoneyElem.innerText.replace(/ VND$/, "")
      );
...
async function Billstatus(timeid, Ngay_mua) {
  ...
  if (element) {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/bills/${timeid}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
...
function checkBillBoxAdmin() {
  if (localStorage.getItem("userlogin")) {
    ...
    if (user.username == "admin") {
      document.getElementById(
        "isAdminBillBox"
      ).innerHTML = `<div class="tim_kiem_theo_ma"> Tìm kiếm theo mã đơn
...
async function openListBill(timeId) {
  try {
    const response = await fetch(
      `http://localhost:3000/api/bill-items/${timeId}`
    );
    const result = await response.json();
    ...
    if (result.success) {
      const itemList = result.data
        ...
        .join("<br>");
      const userBillItemElement = document.getElementById(
        `user_bill_item_${timeId}`
      );
...
async function sap_xep_theo_ngay_tang() {
  await sap_xep_hoa_don(
    (a, b) => new Date(a.date_bill) - new Date(b.date_bill)
  );
}
async function sap_xep_theo_ngay_giam() {
  await sap_xep_hoa_don(
    (a, b) => new Date(b.date_bill) - new Date(a.date_bill)
  );
}
  ...
  try {
    ...
    if (result.success) {
      ...
      if (filteredBills.length === 0) {
        menuList.innerHTML =
          "<tr><td colspan='8'>Không có hóa đơn nào trong khoảng thời gian này.</td></tr>";
      }
    } else {
...
function createTableRow(billItem) {
  ...
  row.innerHTML = `
        ...
        <td>
            ...
            >
                <option value="1" ${
                  billItem.trang_thai === "Vừa đặt" ? "selected" : ""
                }>Vừa đặt</option>
                <option value="2" ${
                  billItem.trang_thai === "2" ? "selected" : ""
                }>Đang giao</option>
        ...
        <td>
            <button class="delete-button" onclick="deleteRow_dh('${
              billItem.timeid
            }')">
                <img src="./img/trash.svg" style="width:20px;height:20px;">
            </button>
...
async function updateBillStatus(timeid, status) {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/api/bills/${timeid}/status`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
...
function thong_ke_doanh_thu() {
  ...
  if (user && allBills) {
    ...
    for (let i = 0; i < user.length; i++) {
      if (allBills[user[i].username]) {
        ...
        for (let j = 0; j < bill.length; j++) {
          ...
          if (
            trang_thai &&
            (trang_thai === "Đã giao" || trang_thai === "Đã nhận")
          ) {
            const total_money = parseFloat(
              bill[j].total_money.replace(" VND", "")
            ); // Xóa dấu phẩy để chuyển thành số
    ...
    document.getElementById("so_luong_san_pham_da_ban").innerText =
      so_luong_da_ban;
    document.getElementById("doanh_thu_san_pham_da_ban").innerText =
      doanh_thu.toLocaleString() + " VND";
  } else {
...
function doanh_thu_trong_ngay() {
  ...
  const menuList = document.querySelector(
    "#page_dt #DTTN #doanh_thu_trong_ngay"
  );
  menuList.innerHTML = ``;
  for (let i = 0; i < user.length; i++) {
    ...
    if (username && allBills[username]) {
      ...
      for (let j = 0; j < userBills.length; j++) {
        ...
        if (
          bill &&
          (bill.trang_thai === "Đã nhận" || bill.trang_thai === "Đã giao")
        ) {
          ...
          if (j == 0) {
            ...
            userBills[j - 1] &&
            bill.timeid !== userBills[j - 1].timeid
          ) {
            dateMap[ngay_mua].so_don_trong_ngay++;
          }
  ...
  for (const ngay_mua in dateMap) {
    if (dateMap.hasOwnProperty(ngay_mua)) {
      ...
      addcontent.innerHTML = `
                        ...
                        <th style="width: 25%;">${
                          infoNgay.so_don_trong_ngay
                        }</th>
                        <th>${
                          infoNgay.doanh_thu.toLocaleString() + " VND"
                        }</th>`;
...
async function lay_nguyen_lieu() {
  try {
    const response = await fetch("http://localhost:3000/api/kho");
    if (!response.ok) {
      throw new Error("Lỗi khi lấy dữ liệu từ API");
    }
    const data = await response.json();

    window.khoData = data; // Lưu dữ liệu vào biến global
    console.log(data);
    console.log("Dữ liệu đã được lấy từ API.");
  } catch (error) {
    console.error("Lỗi khi đọc dữ liệu từ API:", error);
...
async function lay_nguyen_lieu_tu_api() {
  // Nếu chưa có dữ liệu, gọi lại hàm lay_nguyen_lieu để lấy dữ liệu
  if (!window.khoData) {
    await lay_nguyen_lieu(); // Gọi bất đồng bộ và đợi dữ liệu
  }

  // Trả về dữ liệu từ biến global
  return window.khoData || [];
}
...
async function nhap_hang() {
  let tongTien = document.getElementById("bt-total");
  let total = tongTien.value.trim();
  ...
  let rows = document.querySelectorAll("#table-ctpn tr");
  let chiTiet = Array.from(rows)
    .map((row) => {
      let cells = row.getElementsByTagName("td");
      if (cells.length >= 4) {
        let maNguyenLieu = parseInt(cells[0].textContent.trim());
        let donGia = parseInt(cells[2].textContent.trim());
        let soLuong = parseInt(cells[3].textContent.trim());

        if (!isNaN(maNguyenLieu) && !isNaN(soLuong) && !isNaN(donGia)) {
          return { maNguyenLieu, soLuong, donGia };
        }
      }
      return null;
    })
  ...
  const now = new Date();
  const ngayNhap = new Date(now.getTime() + 7 * 60 * 60 * 1000) // Giờ Việt Nam
    .toISOString()
    .replace("T", " ")
    .substring(0, 19);

  try {
    // 1️⃣ Gửi yêu cầu thêm `phieunhap`
    let response1 = await fetch("http://localhost:3000/api/add-Phieunhap", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ tongGia: total, ngayNhap }),
    });

    let data1 = await response1.json();
    if (!response1.ok) {
      throw new Error(data1.message || "Lỗi thêm phiếu nhập");
    }

    let maPhieuNhap = data1.maPhieuNhap; // Nhận ID vừa tạo
    console.log("Mã phiếu nhập:", maPhieuNhap);

    // 2️⃣ Gửi yêu cầu thêm `chitietphieunhap`
    let response2 = await fetch(
      "http://localhost:3000/api/add-ChiTietPhieuNhap",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ maPhieuNhap, chiTiet }),
      }
    );

    let data2 = await response2.json();
    if (!response2.ok) {
      throw new Error(data2.message || "Lỗi thêm chi tiết phiếu nhập");
    }

    let table = document.getElementById("table-ctpn");
    table.innerHTML = "";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  addBtn.addEventListener("click", function () {
    ...
    const selectedRowInList = listNLR.querySelector(
      `tr[data-ten-nguyen-lieu="${tenNguyenLieu}"]`
    );
    console.log(listNLR);
...
removeBtn.addEventListener("click", function () {
  if (selectedRow) {
    ...
    recipeDetails = recipeDetails.filter(
...
async function addProductToDB() {
  try {
    ...
    if (
      ...
      throw new Error(
        "Vui lòng nhập đầy đủ thông tin sản phẩm và thêm ít nhất một nguyên liệu!"
      );
    }
    ...
    const chiTietResponse = await fetch(
      "http://localhost:3000/api/chitietcongthuc",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (!chiTietResponse.ok) {
      throw new Error(
        (await chiTietResponse.text()) || "Lỗi thêm chi tiết công thức"
      );
    }
...
async function loadPhieuNhap() {
  try {
    let response = await fetch("http://localhost:3000/api/hien-phieu-nhap");
    if (!response.ok) throw new Error("Lỗi khi tải dữ liệu phiếu nhập");

    let data = await response.json();
    let tableBody = document.getElementById("table-PN");
    ...
    data.forEach((item) => {
      ...
      row.innerHTML = `
        ...
        <td>${formatCurrency(item.tongGia)}</td>
      `;

      // Thêm sự kiện click để highlight hàng
      row.addEventListener("click", function () {
        // Xóa lớp 'selected-row' khỏi tất cả các hàng
        document
          .querySelectorAll("#table-PN tr")
...
function modal_PN_close() {
  document.getElementById("popup-PN").style.display = "none";
}

async function modal_PN_open() {
  ...
  let maPhieuNhap = selectedRow.children[0].textContent; // Lấy mã phiếu nhập

  // Hiển thị thông tin phiếu nhập
  document.getElementById("ma-phieu-nhap").textContent =
    "Mã phiếu nhập: " + maPhieuNhap;
  document.getElementById("thoi-gian-tao").textContent =
    "Thời gian tạo: " + selectedRow.children[1].textContent;
  document.getElementById("tong-tien").textContent =
    "Tổng tiền: " + selectedRow.children[2].textContent;

  try {
    let response = await fetch(
      `http://localhost:3000/api/chitiet-phieunhap?maPhieuNhap=${maPhieuNhap}`
    );
    if (!response.ok) throw new Error("Lỗi tải dữ liệu chi tiết");

    let data = await response.json();
    let tableBody = document.querySelector("#modal-PN tbody");
    ...
    // Hiển thị popup
    document.getElementById("popup-PN").style.display = "block";
  } catch (error) {
    console.error("Lỗi:", error);
  }
}

// Hàm định dạng thời gian yyyy:mm:dd HH:MM:ss
