...
app.post('/api/congthuc', (req, res) => {
    ...
    if (!tenCongThuc) {
        return res.status(400).json({ message: 'Dữ liệu không hợp lệ: tenCongThuc là bắt buộc' });
    }
    ...
    db2.query(query, values, (err, result) => {
        if (err) {
            ...
            return res.status(500).json({ message: 'Lỗi server khi thêm công thức', error: err.message });
        }
        res.status(201).json({ message: 'Công thức đã được thêm thành công', maCongThuc: result.insertId });
    });
});
...
app.post('/api/chitietcongthuc', (req, res) => {
    ...
    if (!Array.isArray(details) || details.length === 0) {
        return res.status(400).json({ message: 'Dữ liệu không hợp lệ: cần danh sách chi tiết công thức' });
    }

    for (const detail of details) {
        const { maCongThuc, maNguyenLieu, soLuong } = detail;
        if (!maCongThuc || !maNguyenLieu || !soLuong) {
            return res.status(400).json({ message: 'Dữ liệu không hợp lệ: maCongThuc, maNguyenLieu, soLuong là bắt buộc' });
        }
        if (isNaN(soLuong) || soLuong <= 0) {
            return res.status(400).json({ message: 'Dữ liệu không hợp lệ: soLuong phải là số lớn hơn 0' });
        }
    }
    ...
    details.forEach(detail => {
        const { maCongThuc, maNguyenLieu, soLuong } = detail;
        db2.query('SELECT maCongThuc FROM congthuc WHERE maCongThuc = ?', [maCongThuc], (err, congthucRows) => {
            if (err) {
                console.error('Lỗi khi kiểm tra mã công thức:', err);
                errors = true;
                return res.status(500).json({ message: 'Lỗi server khi kiểm tra mã công thức', error: err.message });
            }
            if (congthucRows.length === 0) {
                return res.status(400).json({ message: `Mã công thức ${maCongThuc} không tồn tại` });
            }

            db2.query('SELECT maNguyenLieu FROM kho WHERE maNguyenLieu = ?', [maNguyenLieu], (err, nguyenlieuRows) => {
                if (err) {
                    console.error('Lỗi khi kiểm tra mã nguyên liệu:', err);
                    errors = true;
                    return res.status(500).json({ message: 'Lỗi server khi kiểm tra mã nguyên liệu', error: err.message });
                }
                if (nguyenlieuRows.length === 0) {
                    return res.status(400).json({ message: `Mã nguyên liệu ${maNguyenLieu} không tồn tại` });
                }

                const query = 'INSERT INTO chitietcongthuc (maCongThuc, maNguyenLieu, soLuong) VALUES (?, ?, ?)';
                ...
                db2.query(query, values, (err) => {
                    if (err) {
                        ...
                        errors = true;
                        return res.status(500).json({ message: 'Lỗi server khi thêm chi tiết công thức', error: err.message });
                    }
                    if (!errors && details.length === 1) { // Chỉ gửi response khi hoàn thành tất cả nếu chỉ có 1 bản ghi
                        res.status(201).json({ message: 'Chi tiết công thức đã được thêm thành công' });
                    }
                });
            });
        });
        ...
        details.forEach(detail => {
            ...
            db2.query('SELECT maCongThuc FROM congthuc WHERE maCongThuc = ?', [maCongThuc], (err, congthucRows) => {
                if (err) {
                    console.error('Lỗi khi kiểm tra mã công thức:', err);
                    if (!res.headersSent) {
                        res.status(500).json({ message: 'Lỗi server khi kiểm tra mã công thức', error: err.message });
                    }
                    return;
                }
                if (congthucRows.length === 0) {
                    if (!res.headersSent) {
                        res.status(400).json({ message: `Mã công thức ${maCongThuc} không tồn tại` });
                    }
                    return;
                }
                ...
                db2.query('SELECT maNguyenLieu FROM kho WHERE maNguyenLieu = ?', [maNguyenLieu], (err, nguyenlieuRows) => {
                    if (err) {
                        console.error('Lỗi khi kiểm tra mã nguyên liệu:', err);
                        if (!res.headersSent) {
                            res.status(500).json({ message: 'Lỗi server khi kiểm tra mã nguyên liệu', error: err.message });
                        }
                        return;
                    }
                    if (nguyenlieuRows.length === 0) {
                        if (!res.headersSent) {
                            res.status(400).json({ message: `Mã nguyên liệu ${maNguyenLieu} không tồn tại` });
                        }
                        return;
                    }
                    ...
                    db2.query(query, values, (err) => {
                        if (err) {
                            console.error('Lỗi khi thêm chi tiết công thức:', err);
                            if (!res.headersSent) {
                                res.status(500).json({ message: 'Lỗi server khi thêm chi tiết công thức', error: err.message });
                            }
                            return;
                        }
                        completed++;
...
app.post('/api/sanpham', (req, res) => {
    ...
    if (!tenSanPham || !moTa || !maLoai || !maCongThuc) {
        return res.status(400).json({ message: 'Dữ liệu không hợp lệ: tenSanPham, moTa, maLoai, maCongThuc là bắt buộc' });
    }
    if (isNaN(maLoai) || maLoai <= 0) {
        return res.status(400).json({ message: 'Dữ liệu không hợp lệ: maLoai phải là số lớn hơn 0' });
    }
    if (isNaN(maCongThuc) || maCongThuc <= 0) {
        return res.status(400).json({ message: 'Dữ liệu không hợp lệ: maCongThuc phải là số lớn hơn 0' });
    }

    // Kiểm tra maLoai tồn tại trong bảng loai
    db2.query('SELECT maLoai FROM loai WHERE maLoai = ?', [maLoai], (err, loaiRows) => {
        if (err) {
            console.error('Lỗi khi kiểm tra mã loại:', err);
            return res.status(500).json({ message: 'Lỗi server khi kiểm tra mã loại', error: err.message });
        }
        if (loaiRows.length === 0) {
            return res.status(400).json({ message: `Mã loại ${maLoai} không tồn tại` });
        }
        ...
        db2.query('SELECT maCongThuc FROM congthuc WHERE maCongThuc = ?', [maCongThuc], (err, congthucRows) => {
            ...
            const query = 'INSERT INTO sanpham (tenSanPham, hinhAnh, moTa, maLoai, maCongThuc) VALUES (?, ?, ?, ?, ?)';
            const values = [tenSanPham, hinhAnh || '', moTa, maLoai, maCongThuc];
            db2.query(query, values, (err, result) => {
                if (err) {
                    console.error('Lỗi khi thêm sản phẩm vào MySQL:', err);
                    return res.status(500).json({ message: 'Lỗi server khi thêm sản phẩm', error: err.message });
                }
                res.status(201).json({ message: 'Sản phẩm đã được thêm thành công', maSanPham: result.insertId });
            });
        });
    });
});
...
app.get('/api/nguyenlieu', (req, res) => {
    ...
    db2.query(query, (err, rows) => {
        if (err) {
            ...
            return res.status(500).json({ message: 'Lỗi server khi lấy danh sách nguyên liệu', error: err.message });
        }
        if (rows.length > 0) {
            res.json(rows);
        } else {
...
app.post("/api/register", (req, res) => {
    const { name, email, phone_number, username, password, dateSignup } = req.body;
    ...
    db.query(
        ...
        (err, results) => {
            ...
            const query = "INSERT INTO users (name, email, phone_number, username, password, dateSignup) VALUES (?, ?, ?, ?, ?, ?)";
            db.query(
                query,
                [name, email, phone_number, username, password, dateSignup],
                (err, results) => {
                    if (err) {
                        return res.status(500).json({ success: false, message: "Lỗi khi thêm người dùng!" });
                    }
...
app.get("/api/kho", (req, res) => {
    ...
    db2.query(query, (err, result) => {
        if (err) {
            return res.status(500).json({ error: "Lỗi khi lấy dữ liệu nguyên liệu từ MySQL" });
        }
        res.json(result);
    });
});

app.post('/api/add-Phieunhap', (req, res) => {
    const { ngayNhap, tongGia } = req.body;
    console.log(ngayNhap, tongGia);
    // Xác thực dữ liệu đầu vào
    if (!ngayNhap || !tongGia) {
        return res.status(400).json({ message: 'Dữ liệu không hợp lệ: ngayNhap và tongGia là bắt buộc' });
    }

    // Thực hiện truy vấn SQL để thêm dữ liệu vào bảng
    db2.query(
        ...
        (err, result) => {
            if (err) {
                ...
                return res.status(500).json({ message: 'Lỗi server khi thêm dữ liệu', error: err.message });
            }
            res.status(201).json({ message: 'Dữ liệu nhập đã được thêm thành công', insertId: result.insertId });
        }
    );
});
...
app.post("/api/save-cart", (req, res) => {
    ...
    cart.forEach((item) => {
        const query = "INSERT INTO cart (user_id, menu_item_id, quantity, price) VALUES (?, ?, ?, ?)";
        db2.query(
            query,
...
app.post("/api/add-product", (req, res) => {
    ...
    db2.query("SELECT * FROM menu_items WHERE type = ? AND id = ?", [type, id], (err, existingProducts) => {
        if (err) {
            console.error("Lỗi khi kiểm tra sản phẩm:", err);
            return res.status(500).json({ success: false, message: "Lỗi server, không thể kiểm tra sản phẩm." });
        }
        ...
        console.log("Chạy truy vấn:", query, [type, id, image, title, content, price]);

        db2.query(query, [type, id, image, title, content, price], (err, result) => {
            if (err) {
    ...
    db2.query(
        ...
        (err, orderStats) => {
            if (err) {
                ...
                return res.status(500).json({ success: false, message: "Lỗi server khi lấy thống kê đơn hàng" });
            }
            ...
            db2.query(
                ...
                (err, productStats) => {
                    if (err) {
                        ...
                        return res.status(500).json({ success: false, message: "Lỗi server khi lấy thống kê sản phẩm" });
                    }
                    ...
                    db2.query(
                        ...
                        (err, userStats) => {
                            if (err) {
                                ...
                                return res.status(500).json({ success: false, message: "Lỗi server khi lấy thống kê người dùng" });
                            }
...
app.get("/api/users", (req, res) => {
    ...
    db.query(query, (err, results) => {
        if (err) {
            ...
            return res.status(500).json({ message: "Lỗi khi lấy dữ liệu người dùng" });
        }
...
app.post("/api/bills", (req, res) => {
    const { username, date_bill, timeid, address, van_chuyen, total_money, items } = req.body;

    // Validate input data
    if (!username || !date_bill || !timeid || !address || !van_chuyen || !items || items.length === 0) {
        return res.status(400).json({
            success: false,
    ...
    db2.query("SELECT id FROM users WHERE username = ?", [username], (err, userResult) => {
        if (err) {
            console.error("Lỗi khi lấy user_id:", err);
            return res.status(500).json({ success: false, message: "Lỗi khi lấy user_id" });
        }
        ...
        const billQuery = `
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `;
        db2.query(billQuery, [user_id, username, date_bill, timeid, address, van_chuyen, total_money], (err) => {
            if (err) {
                console.error("Lỗi khi lưu hóa đơn:", err);
                return res.status(500).json({ success: false, message: "Lỗi khi lưu hóa đơn" });
            }
            ...
            db2.query(itemsQuery, [itemsValues], (err) => {
                if (err) {
                    ...
                    return res.status(500).json({ success: false, message: "Lỗi khi lưu chi tiết hóa đơn" });
                }
...
app.put("/api/bills/:timeid/status", (req, res) => {
    ...
    db2.query(sql, [trang_thai, timeid], (err, result) => {
        if (err) {
            ...
            return res.status(500).json({ success: false, message: "Lỗi khi cập nhật trạng thái hóa đơn" });
        }
...
app.get("/api/bills", (req, res) => {
    db2.query("SELECT * FROM bills", (err, results) => {
        if (err) {
            ...
            return res.status(500).json({ success: false, message: "Lỗi khi lấy danh sách hóa đơn" });
        }
...
app.get("/api/bill-items/:timeid", (req, res) => {
    ...
    db2.query("SELECT * FROM bill_items WHERE bill_timeid = ?", [timeid], (err, results) => {
        if (err) {
            console.error("Lỗi khi lấy chi tiết hóa đơn:", err);
            return res.status(500).json({ success: false, message: "Lỗi khi lấy chi tiết hóa đơn" });
        }
        res.status(200).json({ success: true, data: results });
    });
});
...
app.delete("/api/bills/:timeid", (req, res) => {
    ...
    db2.query("DELETE FROM bill_items WHERE bill_timeid = ?", [timeid], (err) => {
        if (err) {
            ...
            return res.status(500).json({ success: false, message: "Lỗi khi xóa chi tiết hóa đơn" });
        }
...
app.get("/api/user-bills/:userId", (req, res) => {
    ...
    db2.query(sql, [userId], (err, rows) => {
        if (err) {
            ...
            return res.status(500).json({ success: false, message: "Unable to fetch user bills." });
        }
...
app.post('/api/upload-image', upload.single('image'), (req, res) => {
    if (!req.file) {
        return res.status(400).json({ message: 'Không có file hình ảnh được tải lên' });
    }
