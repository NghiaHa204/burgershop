...
app.post('/api/register', (req, res) => {
    ...
    db.query('SELECT * FROM users WHERE email = ? OR username = ?', [email, username], (err, results) => {
        ...
        db.query(query, [name, email, phone_number, username, password, dateSignup], (err, results) => {
            if (err) {
...
app.post('/api/save-cart', (req, res) => {
    ...
    cart.forEach(item => {
        ...
        db.query(query, [userId, item.item_id, item.quantity, item.price], (err, result) => {  // Sửa từ 'connection.query' thành 'db.query'
            if (err) {
...
app.post('/api/add-product', async (req, res) => {
    ...
    if (!type || !id || !image || !title || !content || !price) {
        ...
        console.log('Dữ liệu gửi lên không đầy đủ:', { type, id, image, title, content, price });
    ...
    try {
        ...
        res.status(500).json({ success: false, message: 'Lỗi server, không thể thêm sản phẩm.' });
    }
});
...
app.post('/api/bills', async (req, res) => {
    ...
    if (!username || !date_bill || !timeid || !address || !van_chuyen || !items || items.length === 0) {
        return res.status(400).json({ success: false, message: "Thiếu thông tin hóa đơn hoặc sản phẩm" });
    }
    ...
    try {
        ...
        const billQuery = `
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `;
        await db.promise().query(billQuery, [user_id, username, date_bill, timeid, address, van_chuyen, total_money]);
        ...
        res.status(500).json({ success: false, message: "Lỗi khi lưu hóa đơn", error: error.message });
    }
});
...
app.put('/api/bills/:timeid/status', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Lỗi khi cập nhật trạng thái hóa đơn" });
    }
});
...
app.get('/api/bill-items/:timeid', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Lỗi khi lấy chi tiết hóa đơn" });
    }
});
...
app.delete('/api/bills/:timeid', async (req, res) => {
    ...
    try {
        ...
        await db.promise().query('DELETE FROM bill_items WHERE bill_timeid = ?', [timeid]);
...
app.get("/api/user-bills/:userId", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "Unable to fetch user bills." });
    }
});
